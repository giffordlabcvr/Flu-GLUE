...
var ncbiNuccoreSeg4;
var whereClause = "source.name = 'idv-ncbi-nuccore' and rec_segment = 4";
...
_.each(ncbiNuccoreSeg4, function(ncbiNuccoreSeg4) {
	...
	var subtypeResults;
	glue.inMode("/module/idvSeg4MaxLikelihoodGenotyper", function() {
		subtypeResults = glue.command(["genotype", "sequence", "-w", whereClause]);
		//glue.log("INFO", "IDV subtype RESULT WAS ", subtypeResults);			
	});
	...
	if (subtypeResult) {
		...
		var subtype = subtypeResult.replace("AL_IDV_SEG4_", "");
